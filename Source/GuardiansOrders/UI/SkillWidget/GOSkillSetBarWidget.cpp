...
void UGOSkillSetBarWidget::InitializeSkillSlots(UGOSkills* SkillSet)
{
    ...
    for (const auto& Skill : Skills)
    {
        ...
        AGOCharacterBase* OwnerCharacter = Cast<AGOCharacterBase>(GetOwningPlayerPawn()); // ¿ß¡¨¿ª º“¿Ø«œ∞Ì ¿÷¥¬ ƒ≥∏Ø≈Õ∏¶ ∞°¡Æø»
        if (!OwnerCharacter) return;

        UGOCharacterStatComponent* CharacterStatComp = OwnerCharacter->GetStatComponent(); // ƒ≥∏Ø≈Õ¿« Ω∫≈» ƒƒ∆˜≥Õ∆Æ∏¶ ∞°¡Æø»
